...
void publish(GoosePublisher publisher)
{
   ...
   MmsValue* value;
    for (LinkedList element = dataSetValues; element != NULL; element = LinkedList_getNext(element)) {
        value = (MmsValue*)LinkedList_getData(element);
        char buffer[1024];
...
int main(int argc, char **argv)
{
    ...
    while (running && count < 15)
    {
        ...
        publish(publisher);
        printf("Source: VMware_4d:cf:0d\nDestination: IecTc57_01:00:01\n")
        count++;
