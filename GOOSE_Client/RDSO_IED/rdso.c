...
void api_update(const char* timestamp, uint32_t stNum, const char* allData) {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, 100L);  // Set a short timeout
...
void publish(GoosePublisher publisher) {
    ...
    if (GoosePublisher_publish(publisher, dataSetValues) == -1) {
        ...
        log_debug("GOOSE message sent successfully: Status: %s StNum: %d, t: %s", statusBool ? "True" : "False", stNum, published_timestamp_str);
    }
